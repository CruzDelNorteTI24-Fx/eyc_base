...
<head>
    ...
    <title>Historial de stocks | Eyc</title>
    ...
    <link rel="stylesheet" href="<?= eyc_asset('assets/css/inventario_stock_eyc.css') ?>">
</head>
<body>
<?php eyc_render_sidebar(); ?>
<?php eyc_render_header(['title' => 'Historial de stocks', 'subtitle' => 'Saldo inicial, entradas, salidas y final']); ?>

<div class="eyc-main">
    <?php eyc_render_content_separator('top'); ?>

    <main class="eyc-content eyc-stock-page">
...
<script>
window.eyc_STOCK_REPORT = {
    historyEndpoint: '<?= hist_h(eyc_base_url('php/ver_movimientos_producto.php')) ?>',
    noteEndpoint: '<?= hist_h(eyc_base_url('php/ver_nota_salida.php')) ?>',
    pdf: {
        ...
        logoLeft: '<?= hist_h(eyc_base_url('img/icon.png')) ?>',
        logoRight: '<?= hist_h(eyc_base_url('img/eyc.png')) ?>',
        useCover: false,
        fileName: 'historial_stocks_<?= date('Ymd_His') ?>.pdf'
    }
};
window.eyc_NOTA_PDF_CONFIG = {
    endpoint: '<?= hist_h(eyc_base_url('php/nota_pdf_data.php')) ?>',
    userName: <?= json_encode((string)($_SESSION['usuario'] ?? ''), JSON_UNESCAPED_UNICODE) ?>,
    dni: <?= json_encode((string)($_SESSION['DNI'] ?? ''), JSON_UNESCAPED_UNICODE) ?>,
    logoTicket: '<?= hist_h(eyc_base_url('img/completo2.png')) ?>',
    footerLabel: 'Eyc'
};
</script>
...
<script src="<?= eyc_asset('assets/js/inventario_stock_eyc.js') ?>"></script>
