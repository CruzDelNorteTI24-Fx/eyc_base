...
<?php eyc_render_header(['title' => 'Inventario', 'subtitle' => 'Control operativo de stock']); ?>
...
window.eyc_STOCK_REPORT = {
    historyEndpoint: '<?= inv_h(eyc_base_url('php/ver_movimientos_producto.php')) ?>',
    noteEndpoint: '<?= inv_h(eyc_base_url('php/ver_nota_salida.php')) ?>',
    pdf: {
        ...
        logoLeft: '<?= inv_h(eyc_base_url('img/icon.png')) ?>',
        logoRight: '<?= inv_h(eyc_base_url('img/eyc.png')) ?>',
        useCover: false,
        fileName: 'inventario_productos_<?= date('Ymd_His') ?>.pdf'
    }
};
window.eyc_NOTA_PDF_CONFIG = {
    endpoint: '<?= inv_h(eyc_base_url('php/nota_pdf_data.php')) ?>',
    userName: <?= json_encode((string)($_SESSION['usuario'] ?? ''), JSON_UNESCAPED_UNICODE) ?>,
    dni: <?= json_encode((string)($_SESSION['DNI'] ?? ''), JSON_UNESCAPED_UNICODE) ?>,
    logoTicket: '<?= inv_h(eyc_base_url('img/completo2.png')) ?>',
    footerLabel: 'Eyc'
};
</script>
