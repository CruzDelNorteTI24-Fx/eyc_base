...
function eyc_qs_clean_output(): void {
    if (ob_get_length()) {
        ob_clean();
    }
}

function eyc_qs_json(array $payload, int $status = 200): void {
    eyc_qs_clean_output();
...
function eyc_qs_text($value, string $fallback = '-'): string {
    $text = trim((string)($value ?? ''));
    return $text !== '' ? $text : $fallback;
}

function eyc_qs_number($value): string {
    if ($value === null || $value === '') {
        return '-';
    }
...
function eyc_qs_stock_level($stock, string $estado): string {
...
function eyc_qs_product_payload(array $row): array {
    $codigoCategoria = eyc_qs_text($row['codigo_categoria'] ?? '', '');
    $categoriaBase = eyc_qs_text($row['categoria_descripcion'] ?? '', '') ?: eyc_qs_text($row['categoria'] ?? '', 'Sin categoria');
    $categoria = trim(($codigoCategoria !== '' ? '(' . $codigoCategoria . ') ' : '') . $categoriaBase);
    $stock = $row['Stock_Actual'] ?? null;
    $estado = eyc_qs_text($row['Estado'] ?? 'Sin estado', 'Sin estado');

    return [
        'id' => (int)($row['id_producto'] ?? 0),
        'codigo' => eyc_qs_text($row['codigo_producto'] ?? ''),
        'nombre' => eyc_qs_text($row['producto_nombre'] ?? ''),
        'unidad' => eyc_qs_text($row['unidproducto'] ?? '', ''),
        'categoria' => $categoria,
        'stock' => eyc_qs_number($stock),
        'estado' => $estado,
        'stock_level' => eyc_qs_stock_level($stock, $estado),
    ];
}
...
if ($isEtiqueta) {
    ...
    if ($row) {
        eyc_qs_json([
            ...
            'mode' => 'etiqueta',
            'code' => $query,
            'product' => eyc_qs_product_payload($row),
            'trace' => [
                'sede' => eyc_qs_text($row['sede_nombre'] ?? ''),
                'anaquel' => eyc_qs_text($row['anaquel'] ?? ''),
                'fecha_ingreso' => eyc_qs_text($row['fecha_movimiento'] ?? ''),
                'fecha_etiquetado' => eyc_qs_text($row['fecha_etiquetado'] ?? ''),
                'estado_etiqueta' => eyc_qs_text($row['estado_etiqueta'] ?? ''),
                'cantidad_movimiento' => eyc_qs_number($row['cantidad_movimiento'] ?? null),
                'observacion' => eyc_qs_text($row['observacion_movimiento'] ?? ''),
            ],
        ]);
    }
}
...
while ($item = $result->fetch_assoc()) {
    $suggestions[] = eyc_qs_product_payload($item);
}
