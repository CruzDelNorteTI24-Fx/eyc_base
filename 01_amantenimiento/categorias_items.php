    ...
    <button type="submit" class="btn">Añadir Categoría</button>
      ...
      <table>
        <tr>
          ...
          <th style="background: #2c3e50;">Categoría</th>
      ...
      while ($cat = $res->fetch_assoc()) {
        ...
        if ($res_items->num_rows > 0) {
          echo "<table>
            <tr>
              <th style='background: #2c3e50;'>ID</th>
              <th style='background: #2c3e50;'>Ítem</th>
              <th style='background: #2c3e50;'>CheckList</th>
              <th style='background: #2c3e50;'>Tipo</th>
              <th style='background: #2c3e50;'>Acciones</th>
            </tr>";
          while ($item = $res_items->fetch_assoc()) {
            ...
            echo "<tr>
              ...
              <td>
                <form action='item/asignar_tipo.php' method='POST'>
                  <input type='hidden' name='item_id' value='{$item['clm_item_id']}'>
                  <select name='tipo_id' class='input-evaluacion' onchange='this.form.submit()'>
                    <option value=''>-- Seleccionar tipo --</option>";
                    foreach ($tipos_array as $tipo) {
                      // Si deseas marcar el actual como seleccionado, obtén el campo en la BD correspondiente
                      $selected = ($item['clm_item_idtipocheck'] == $tipo['clm_checktip_id']) ? "selected" : "";
                      echo "<option value='{$tipo['clm_checktip_id']}' $selected>{$tipo['clm_checktip_nombre']}</option>";
                    }
              echo "</select>
                </form>
              </td>
                <td>
                  <form action='item/actualizar_tipo_item.php' method='POST'>
                    <input type='hidden' name='item_id' value='{$item['clm_item_id']}'>
                    <select name='nuevo_tipo' class='input-evaluacion' onchange='this.form.submit()'>
                      <option value='R' ".($item['clm_items_tipo']=='R'?'selected':'').">Radio - C/NC/NA</option>
                      <option value='E' ".($item['clm_items_tipo']=='E'?'selected':'').">Radio - Empresa(E)/Propio(P)</option>
                      <option value='Q' ".($item['clm_items_tipo']=='Q'?'selected':'').">Radio - Requiere(Q)/NoRequiere(NQ)/No Aplica(NP)</option>
                      <option value='H' ".($item['clm_resultado_dfecd']=='H'?'selected':'').">Fecha y Hora</option>
                      <option value='T' ".($item['clm_items_tipo']=='T'?'selected':'').">Texto Lista de Conductores</option>
                      <option value='O' ".($item['clm_items_tipo']=='O'?'selected':'').">Texto Libre</option>
                      <option value='N' ".($item['clm_items_tipo']=='N'?'selected':'').">Número</option>
                      <option value='F' ".($item['clm_items_tipo']=='F'?'selected':'').">Foto</option>
                      <option value='D' ".($item['clm_items_tipo']=='D'?'selected':'').">Documento</option>
                    </select>
                  </form>
                </td>
            <td class='acciones-btns'>
...
<div id="modalEditarItem" class="modal">
  <div class="modal-content">
    <span class="cerrar" onclick="cerrarModalEditarItem()">&times;</span>
    <h2>Editar Ítem</h2>
    <form action="item/editar_item.php" method="POST">
      <input type="hidden" name="id" id="editarItemId">
      <input type="text" name="item_nombre" id="editarItemNombre" required>
      <button type="submit" class="btn">Guardar Cambios</button>
    </form>
  </div>
</div>
<script>
function abrirModalEditarItem(id, nombre) {
  document.getElementById('editarItemId').value = id;
...
<div id="modalEditarCategoria" class="modal">
  <div class="modal-content">
    ...
    <form action="categoria/editar_categoria.php" method="POST">
      <input type="hidden" name="id" id="editarCategoriaId">
      <input type="text" name="categoria_nombre" id="editarCategoriaNombre" required>
      <button type="submit" class="btn">Guardar Cambios</button>
    </form>
  </div>
</div>
