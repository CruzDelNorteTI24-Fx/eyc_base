...
.fila-seleccionada {
  background-color: #d1ecf1 !important; /* celeste suave */
}
...
<h2>Gestión de Placas</h2>
<div class="boton-agregar">
  <a href="nueva_placa.php" class="btn-agregar-placa">➕ Registrar Nueva Placa</a>
</div>

<div class="filtros">
  <div class="filtro-campo">
    <label for="filtroNombre">Nombre del Bus</label>
    <input type="text" id="filtroNombre" placeholder="Ej. 1000">
  </div>
  <div class="filtro-campo">
    <label for="filtroPlaca">Placa</label>
    <input type="text" id="filtroPlaca" placeholder="Ej. ABC-123">
  </div>
  <div class="filtro-campo">
    <label for="filtroDueño">Dueño</label>
    <input type="text" id="filtroDueño" placeholder="Ej. Sr. Eddy">
  </div>
  <div class="filtro-campo">
    <label for="filtroServicio">Servicio</label>
    <input type="text" id="filtroServicio" placeholder="Ej. Primera Clase">
  </div>
</div>
...
<tbody>
  ...
  <tr>
    <td>
  <button class="btn-seleccionar" onclick="seleccionarFila(this)">➤</button>
</td>

    <td><?= htmlspecialchars($row['clm_placas_id']) ?></td>
    <td><?= htmlspecialchars($row['clm_placas_PLACA']) ?></td>

    <!-- Dueño con color -->
    <td>
      <?php
        $dueño = htmlspecialchars($row['clm_placas_DUEÑO']);
        if (strtolower($dueño) == "sr. eddy") {
          echo '<span class="dueño-eddy">'.$dueño.'</span>';
        } else {
          echo $dueño;
        }
      ?>
    ...
    <td><?= htmlspecialchars($row['clm_placas_BUS']) ?></td>
    <td><?= htmlspecialchars($row['clm_placas_TIPO_VEHÍCULO']) ?></td>

    <!-- Estado con color -->
    <td>
      <?php
        $estado = htmlspecialchars($row['clm_placas_ESTADO']);
        if (strtolower($estado) == "activo") {
          echo '<span class="estado-activo">Activo</span>';
        } elseif (strtolower($estado) == "inactivo") {
          echo '<span class="estado-inactivo">Inactivo</span>';
        } else {
          echo $estado;
        }
      ?>
    ...
    <td><?= number_format($row['clm_placas_KILOMETRAJE']) ?> km</td>

    <!-- Servicio con color -->
    <td>
      <?php
        $servicio = htmlspecialchars($row['clm_placas_servicio']);
        if (strtolower($servicio) == "transporte") {
          echo '<span class="servicio-transporte">'.$servicio.'</span>';
        } elseif (strtolower($servicio) == "mantenimiento") {
          echo '<span class="servicio-mantenimiento">'.$servicio.'</span>';
        } else {
          echo $servicio;
        }
      ?>
    ...
    <!-- Acciones -->
    <td>
      <a href="javascript:void(0);" style="color:#2980b9; font-weight:bold;"
        onclick="abrirModalEditar(
          '<?= $row['clm_placas_id'] ?>',
          '<?= htmlspecialchars($row['clm_placas_PLACA'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_DUEÑO'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_BUS'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_TIPO_VEHÍCULO'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_ESTADO'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_KILOMETRAJE'], ENT_QUOTES) ?>',
          '<?= htmlspecialchars($row['clm_placas_servicio'], ENT_QUOTES) ?>'
        )">Editar</a> |
      <a href="eliminar_placa.php?id=<?= $row['clm_placas_id'] ?>" style="color:#c0392b; font-weight:bold;" onclick="return confirm('¿Estás seguro de inhabilitar esta placa?');">Inhabilitar</a>
    </td>
  </tr>
  <?php endwhile; ?>
</tbody>
...
function seleccionarFila(btn) {
  ...
  fila.classList.toggle('fila-seleccionada'); // alterna selección individual
}
</script>
...
<div id="modalEditarPlaca" class="modal">
  <div class="modal-content">
    ...
    <form id="formEditarPlaca" action="placas/actualizar_placa.php" method="POST">
      <input type="hidden" name="clm_placas_id" id="edit_id">

      <div class="campo-form">
        <label for="edit_placa">Placa:</label>
        <input type="text" id="edit_placa" name="clm_placas_PLACA" required>
      </div>

      <div class="campo-form">
        <label for="edit_dueño">Dueño:</label>
        <input type="text" id="edit_dueño" name="clm_placas_DUEÑO" required>
      </div>

      <div class="campo-form">
        <label for="edit_bus">Bus:</label>
        <input type="text" id="edit_bus" name="clm_placas_BUS" required>
      </div>

      <div class="campo-form">
        <label for="edit_tipo">Tipo Vehículo:</label>
        <input type="text" id="edit_tipo" name="clm_placas_TIPO_VEHÍCULO" required>
      </div>

      <div class="campo-form">
        <label for="edit_estado">Estado:</label>
        <input type="text" id="edit_estado" name="clm_placas_ESTADO" required>
      </div>

      <div class="campo-form">
        <label for="edit_kilometraje">Kilometraje:</label>
        <input type="text" id="edit_kilometraje" name="clm_placas_KILOMETRAJE" required>
      </div>

      <div class="campo-form">
        <label for="edit_servicio">Servicio:</label>
        <input type="text" id="edit_servicio" name="clm_placas_servicio" required>
      </div>
...
<script>
function abrirModalEditar(id, placa, dueño, bus, tipo, estado, kilometraje, servicio) {
  ...
  document.getElementById('edit_kilometraje').value = kilometraje;
  document.getElementById('edit_servicio').value = servicio;

  document.getElementById('modalEditarPlaca').style.display = 'block';
}

function cerrarModalEditar() {
  document.getElementById('modalEditarPlaca').style.display = 'none';
}
</script>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const filtroDueño = document.getElementById("filtroDueño");
  ...
  function aplicarFiltro() {
    ...
    const valorPlaca = filtroPlaca.value.toLowerCase().replace(/-/g, '');
    const valorDueño = filtroDueño.value.toLowerCase();
    const valorServicio = filtroServicio.value.toLowerCase();

    filas.forEach(fila => {
      const nombreBus = fila.cells[4]?.textContent.toLowerCase() || "";
      const placaRaw = fila.cells[2]?.textContent.toLowerCase() || "";
      const placaSinGuion = placaRaw.replace(/-/g, '');
      const dueño = fila.cells[3]?.textContent.toLowerCase() || "";
      const servicio = fila.cells[8]?.textContent.toLowerCase() || "";

      const coincide =
        nombreBus.includes(valorNombre) &&
        placaSinGuion.includes(valorPlaca) &&
        dueño.includes(valorDueño) &&
        servicio.includes(valorServicio);
  ...
  filtroDueño.addEventListener("input", aplicarFiltro);
