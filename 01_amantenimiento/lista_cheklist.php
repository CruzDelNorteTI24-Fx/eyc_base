...
// Todos los checklists generados en la fecha seleccionada ($fecha_mostrar) para todos los tipos de checklist.
//Mostrar todos los checklists en la vista de cards de Admin.
  // Definir $fecha_mostrar con fecha de hoy si no está seteada o si se envía por POST
  $fecha_mostrar = isset($_POST['fecha_mostrar']) ? $_POST['fecha_mostrar'] : date('Y-m-d');
  // Consulta de checklists generados en la fecha seleccionada
  if ($_SESSION['web_rol'] === 'Admin') {
      // Admin ve todos los tipos
      $stmt_gen = $conn->prepare("SELECT c.clm_checklist_id, c.clm_checklist_corr, c.clm_checklist_idtipo, c.clm_checklist_fecha, c.clm_checklist_hora,
          t.clm_checktip_nombre,
          p.clm_placas_placa, p.clm_placas_BUS,
          IF(c.clm_checklist_fecha = CURDATE(), 'Abierto', 'Cerrado') AS estado_checklist
      FROM tb_checklist_limpieza c
      JOIN tb_placas p ON c.clm_checklist_id_bus = p.clm_placas_id
      LEFT JOIN tb_checklist_tipos t ON c.clm_checklist_idtipo = t.clm_checktip_id
      WHERE c.clm_checklist_fecha = ?
      ORDER BY p.clm_placas_BUS");
      $stmt_gen->bind_param("s", $fecha_mostrar);
      $stmt_gen->execute();
...
// Restringe los tipos de checklist que puede ver el usuario en la fecha seleccionada.

$sql = "SELECT c.clm_checklist_id, c.clm_checklist_corr, c.clm_checklist_idtipo, c.clm_checklist_fecha, c.clm_checklist_hora,
        t.clm_checktip_nombre,
        p.clm_placas_placa, p.clm_placas_BUS,
        IF(c.clm_checklist_fecha = CURDATE(), 'Abierto', 'Cerrado') AS estado_checklist
    FROM tb_checklist_limpieza c
    ...
    LEFT JOIN tb_checklist_tipos t ON c.clm_checklist_idtipo = t.clm_checktip_id
    WHERE c.clm_checklist_fecha = ?
      AND c.clm_checklist_idtipo IN (".implode(',', array_fill(0, count($tipoChecklist), '?')).")
    ORDER BY p.clm_placas_BUS";

$stmt_gen = $conn->prepare($sql);

$types = str_repeat('i', count($tipoChecklist));
$stmt_gen->bind_param("s".$types, $fecha_mostrar, ...$tipoChecklist);
$stmt_gen->execute();
$res_gen = $stmt_gen->get_result(); 
...
<div class="menu-lateral" id="menuLateral">
    <?php
        ...
        if ($_SESSION['web_rol'] == 'Admin' || in_array("c-sab", $vistas)) {
          echo "<h3>Generar CheckList</h3>";
          echo "<ul>";
            echo "<li><a href='limpieza/mantcdn.php?id_tipo=2'>➕ Nuevo SAB</a></li>";
          echo "</ul>";
        }
    ?>
  ...
  <ul>
    <?php
        if ($_SESSION['web_rol'] == 'Admin') {
          ...
          echo "<li><a href='gest_plac.php'>Gestionar Placas</a></li>";
        }
...
<form method="POST" style="text-align:center; margin-bottom:30px; display:flex; flex-wrap:wrap; justify-content:center; align-items:center; gap:10px;">
  <label for="fecha_mostrar" style="font-weight:bold; color:#2c3e50;">Seleccionar fecha:</label>
  <input type="date" name="fecha_mostrar" id="fecha_mostrar" value="<?= $fecha_mostrar ?>" style="padding:8px 10px; border-radius:8px; border:1px solid #ccc; font-size:15px;">
  <button type="submit" style="background:linear-gradient(120deg,#2980b9,#3498db); color:white; border:none; padding:8px 20px; border-radius:8px; font-weight:bold; cursor:pointer; transition:background 0.3s;">Buscar</button>
</form>


<h3 style="text-align:center; margin-bottom:30px;">
  Checklists generados en <?= date('d/m/Y', strtotime($fecha_mostrar)) ?>
</h3>

<div style="text-align:center; margin-bottom: 30px;">
  <input type="text" id="buscador-checklist" class="input-evaluacion" placeholder="Buscar por placa, bus, estado..." onkeyup="filtrarChecklists()" style="max-width:400px;">
</div>

<div style="grid-template-columns:repeat(auto-fit, minmax(280px,1fr)); gap:20px; max-width:1100px; margin:0 auto;">
<?php
if ($res_gen->num_rows > 0) {
    // Agrupar por tipo
    $checklists_por_tipo = [];
    while ($row = $res_gen->fetch_assoc()) {
        $tipo = $row['clm_checktip_nombre'] ?? 'Sin Tipo';
        if (!isset($checklists_por_tipo[$tipo])) {
            $checklists_por_tipo[$tipo] = [];
        }
        $checklists_por_tipo[$tipo][] = $row;
    }

    echo "<div class='checklist-grid'>";

    foreach ($checklists_por_tipo as $tipo_nombre => $checklists) {
        echo "<div class='checklist-tipo-titulo'>".htmlspecialchars($tipo_nombre)."</div>";
...
//Cuenta los items totales y respondidos de un checklist específico.
              ...
              SELECT COUNT(*) as total, 
                    SUM(
                      CASE 
                        ...
                        WHEN i.clm_items_tipo = 'T' THEN r.clm__rescheck_conductor1 IS NOT NULL AND r.clm__rescheck_conductor1 != ''
                        WHEN i.clm_items_tipo = 'O' THEN r.clm__rescheck_conductor1 IS NOT NULL AND r.clm__rescheck_conductor1 != ''
            ...
            $estado_completitud  = ($total == $respondidos) ? "Completo" : "Incompleto";
            $color_estado = ($estado_completitud =="Completo") ? "#27ae60" : "#c0392b";
            // Obtener estado abierto/cerrado calculado en el SELECT
            $estado_abierto = $row['estado_checklist']; // 'Abierto' o 'Cerrado'

            // Definir clase adicional y texto botón según abierto/cerrado
            $clase_card_extra = ($estado_abierto == "Cerrado") ? "background:#e0e0e0; opacity:0.8;" : "";
            $texto_btn = ($estado_abierto == "Cerrado") ? "Ver" : "Ver / Llenar";

            echo "
            <div class='checklist-card' style='$clase_card_extra'>
              <div style='display:flex; justify-content:space-between; align-items:center;'>
                <h4 style='margin:0; color:#2980b9; font-size:18px;'>".htmlspecialchars($row['clm_placas_BUS'])."</h4>
                <span class='checklist-estado' style='background:$color_estado;'>$estado_completitud</span>
              </div>
              <div style='border-top:1px solid #eee; padding-top:10px;'>
                <p style='margin:4px 0; color:#555; font-size:14px;'><strong>Checklist:</strong> ".htmlspecialchars($row['clm_checklist_corr'])."</p>
                <p style='margin:4px 0; color:#555; font-size:14px;'><strong>Placa:</strong> ".htmlspecialchars($row['clm_placas_placa'])."</p>
                <p style='margin:4px 0; color:#555; font-size:14px;'><strong>Estado:</strong> ".htmlspecialchars($estado_abierto)."</p>
                <p style='margin:4px 0; color:#555; font-size:14px;'><strong>Items:</strong> {$respondidos} / {$total}</p>
    ...
    echo "<p style='text-align:center; color:gray;'>No hay checklists generados en esta fecha.</p>";
}
?>
...
<div id="modalKPI" class="modal">
  <div class="modal-content">
    ...
    <span onclick="cerrarModalKPI()" class="cerrar-modal-kpi">&times;</span>
    <div class="modal-header">
      <h2><img src="../img/icons/kpi.png" alt="KPI" style="width:28px; vertical-align:middle; margin-right:10px;"> Resultado KPI <span style="font-weight:normal; font-size:16px;">&nbsp;&nbsp;#${correlativo}</span></h2>
    </div>
    
    
    <div class="modal-body" id="kpiBody">
      <p id="kpiTitulo" style="font-size: 20px; margin: 10px 0;"><strong>KPI:</strong> </p>
      <p id="kpiTexto" style="font-size: 20px; margin: 10px 0;"><strong>KPI:</strong> </p>
      <p id="kpiValor" style="font-size: 20px; margin: 10px 0;"><strong>Porcentaje C:</strong> </p>
    </div>
...
function formatearFechaISOaDMY(iso) {
  if (!iso) return '';
  // Acepta "YYYY-MM-DD", "YYYY/MM/DD" o "YYYY-MM-DDTHH:MM:SS"
  const limpio = String(iso).trim().split('T')[0];
  const [y, m, d] = limpio.split(/[-\/]/).map(x => String(x).trim());
  if (!y || !m || !d) return '';
  return `${d.padStart(2,'0')}/${m.padStart(2,'0')}/${y.padStart(4,'0')}`;
}

function formatearHoraHMS(hhmmss) {
  if (!hhmmss) return '';
  let str = String(hhmmss).trim();
  const partes = str.split(/[T ]/);
  str = partes[partes.length - 1];
  let [h='0', m='0', s='0'] = str.split(':').map(x => String(x || '').trim());

  // Limpia caracteres no numéricos (e.g. "05.123Z" -> "05")
  h = h.replace(/\D+/g, '');
  m = m.replace(/\D+/g, '');
  s = s.replace(/\D+/g, '');
...
function mostrarKPI(id_checklist, correlativo = "", fecha = "", hora = ""){
  fetch('ver_kpi_checklist.php?id_checklist=' + id_checklist)
    ...
    .then(data => {
      if (data.error) {
        ...
        <div class="kpi-section">
          <p><strong>Indicador:</strong> ${data.kpi_titulo}</p>
         <p><strong>KPI:</strong></p>
        `;

        // Si KPI tipo 1 (Deficiente, Aceptable, Excelente)
        if (["DEFICIENTE", "ACEPTABLE", "EXCELENTE"].includes(data.kpi_texto)) {
          let badgeClass = "";
          switch (data.kpi_texto) {
            case "DEFICIENTE": badgeClass = "badge-deficiente"; break;
            case "ACEPTABLE": badgeClass = "badge-aceptable"; break;
            case "EXCELENTE": badgeClass = "badge-excelente"; break;
          }
          html += `<span class="badge ${badgeClass}">${data.kpi_texto}</span>`;
          } else if (data.kpi_titulo && data.kpi_titulo.includes("Fumigación")) {
            html += `
              <p style="margin-top:10px;font-size:16px;">${data.kpi_texto}</p>
              <p style="margin-bottom:10px;"><strong>Última fecha registrada:</strong> ${data.kpi_valor}</p>
            `;
          } else {
            html += `<p style="margin-top:10px;">${data.kpi_texto}</p>`;
          }


        html += `</div>`;

        // Si es tipo alcoholímetro con varios conductores
        if (Array.isArray(data.kpi_valor)) {
          data.kpi_valor.forEach(conductor => {
            const badgeClass = (conductor.estado === 'APTO') ? 'badge-apto' : 'badge-noapto';
            html += `
              <div class="kpi-section">
                <p><strong>${conductor.nombre}</strong></p>
                <p>Porcentaje: ${conductor.valor}</p>
                <span class="badge ${badgeClass}">${conductor.estado}</span>
            ...
            html += `
            <div class="kpi-section">
              <p><strong>Valor:</strong> ${data.kpi_valor}</p>
            </div>`;
          }

        html += `</div><div class="modal-scroll">`;
        html += `
        <p onclick="toggleItemsDetalle()" style="
          display:inline-block;
          color:#2980b9;
          cursor:pointer;
          font-weight:bold;
          ...
          border-radius:4px;
          width:auto;
        ">+ Ver Items Detallados</p>
        <div id="itemsDetalle" style="display:none; margin-top:15px;">`;
        ...
        if (data.items) {
          data.items.forEach(item => {
            ...
            if (item.tipo === 'F' && item.resultado) {
              html += `<img src="${item.resultado}" alt="Evidencia" style="max-width:100%; max-height:180px; border-radius:8px; margin-top:5px;">`;
            } else {
              html += `<p>Resultado: ${item.resultado || "-"}</p>`;
            }
        ...
        // Inyectar contenido
        document.getElementById('kpiBody').innerHTML = html;

        const fechaDMY = formatearFechaISOaDMY(fecha);
        const horaHMS  = formatearHoraHMS(hora);

        const partes = [`#${correlativo}`, fechaDMY, horaHMS].filter(Boolean);
        document.querySelector("#modalKPI .modal-header h2").innerHTML = `
          <img src="../img/icons/kpi.png" alt="KPI" style="width:28px; vertical-align:middle; margin-right:10px;">
          Resultado KPI
          <div style="font-weight:normal; font-size:16px; line-height:1.2; margin-top:4px;">
            ${partes.join(' &nbsp;·&nbsp; ')}
          </div>`;
    ...
    .catch(error => {
      ...
      alert('Error al obtener el KPI.');
    });
}


function toggleItemsDetalle() {
  const div = document.getElementById('itemsDetalle');
  const btn = event.target;

  if (div.style.display === 'none') {
    div.style.display = 'block';
    btn.textContent = '- Ocultar Items Detallados';
  } else {
    div.style.display = 'none';
    btn.textContent = '+ Ver Items Detallados';
  }
}
...
function filtrarChecklists() {
  const input = document.getElementById('buscador-checklist').value.toLowerCase();
