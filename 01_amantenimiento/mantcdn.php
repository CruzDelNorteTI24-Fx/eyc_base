...
<div class="main-content">
  ...
  $excluir_placas = ['H2U-919', '0']; // placas que NO verá Usuario
...
while ($bus = $buses->fetch_assoc()) {
  ...
  // Verificar si ya tiene checklist hoy
  $stmt = $conn->prepare("SELECT COUNT(*) FROM tb_checklist_limpieza WHERE clm_checklist_id_bus=? AND clm_checklist_fecha=?");
  $stmt->bind_param("is", $id_bus, $hoy);
  $stmt->execute();
...
function seleccionarBus(id,estado) {
  ...
  if (estado.includes('Generado')) {
    mensaje.innerHTML = "⚠️ Este bus ya tiene su checklist generado hoy.";
...
function cargarBuses() {
  ...
  document.getElementById('fecha_form').value = fecha; // actualizar input hidden
...
function cerrarError() {
  ...
  <form action="checklist/guardar_checklist.php" method="POST">
    ...
    <div class="campo-form">
      ...
      <select name="bus" required class="input-evaluacion">
        ...
        <?php
        $consulta_buses = $conn->query("SELECT clm_placas_id, clm_placas_placa FROM tb_placas ORDER BY clm_placas_placa");
