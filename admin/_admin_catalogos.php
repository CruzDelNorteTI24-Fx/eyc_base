...
function eyc_admin_h($value): string {
    return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
}

function eyc_admin_query_all(mysqli $conn, string $sql): array {
...
function eyc_admin_columns(mysqli $conn, string $table): array {
    static $cache = [];
...
function eyc_admin_has_column(mysqli $conn, string $table, string $column): bool {
    return in_array($column, eyc_admin_columns($conn, $table), true);
}

function eyc_admin_module_name($value): string {
...
function eyc_admin_space_module($value): string {
    $id = is_numeric($value) ? (int)$value : 0;
...
function eyc_admin_photo_data_uri($blob): string {
...
function eyc_admin_initials(string $name): string {
...
function eyc_admin_permissions_by_user(mysqli $conn): array {
    $rows = eyc_admin_query_all($conn, "
    ...
    foreach ($rows as $row) {
        ...
        $module = eyc_admin_module_name($row['id_modulo'] ?? 0);
...
function eyc_admin_render_head(string $title): void {
    ?>
    <!DOCTYPE html>
    <html lang="es">
    <head>
        <meta charset="UTF-8">
        <title><?= eyc_admin_h($title) ?> | eyc</title>
        ...
        <link rel="stylesheet" href="<?= eyc_asset('assets/css/admin_catalogos_eyc.css') ?>">
    </head>
    <body>
    <?php
}

function eyc_admin_render_close(): void {
    ?>
