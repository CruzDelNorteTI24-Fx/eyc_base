...
function eyc_doc_h($value): string {
...
<head>
    ...
    <link rel="stylesheet" href="<?= eyc_asset('assets/css/admin_documentacion_eyc.css') ?>">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
</head>
<body>
<?php eyc_render_header(['title' => 'Formatos', 'subtitle' => 'Estandar PDF']); ?>
<?php eyc_render_sidebar(); ?>

<main class="main-content eyc-main eyc-main--module eyc-main--compact-access" role="main">
    <div class="eyc-main__inner admin-doc-shell">
...
<script src="<?= eyc_asset('assets/js/formatos/notas/eyc_notas_demo_data.js') ?>"></script>
...
async function generarDemoPDF(orientation, button) {
    if (!window.eycPDF) {
        throw new Error('El estandar eycPDF no esta cargado.');
    }

    await window.eycPDF.generateDemo(Object.assign({}, pdfConfig, {
        orientation,
        cover: true
    }));
}
...
async function generarNotaDemo(format, button) {
    const generators = {
        NS: window.eycNotaSalidaAlmacen,
        CM: window.eycNotaTanqueada,
        NE: window.eycNotaEntradaAlmacen,
        AB: window.eycNotaAbastecimiento
    };
