...
$tieneSede = eyc_admin_has_column($conn, 'tb_espacio', 'clm_esp_idsede');
$sqlSede = $tieneSede
...
$espacios = eyc_admin_query_all($conn, "
    SELECT
        e.clm_esp_id,
        e.clm_esp_nombre,
        e.clm_esp_desc,
...
foreach ($espacios as $espacio) {
    $modulo = eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0);
    $porModulo[$modulo] = ($porModulo[$modulo] ?? 0) + 1;
}

eyc_admin_render_head('Espacios');
?>
<?php eyc_render_header(['title' => 'Espacios', 'subtitle' => 'Administracion']); ?>
            ...
            <div class="admin-cat-table-wrap">
                <table class="admin-cat-table">
                    ...
                    <?php foreach ($espacios as $espacio): ?>
                        <tr>
                            <td><?= eyc_admin_h($espacio['clm_esp_id'] ?? '') ?></td>
                            <td><span class="admin-cat-chip"><?= eyc_admin_h($espacio['clm_esp_nombre'] ?? '') ?></span></td>
                            <td><div class="admin-cat-main"><strong><?= eyc_admin_h($espacio['clm_esp_desc'] ?? '') ?></strong></div></td>
                            <td><?= eyc_admin_h(eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0)) ?></td>
                            <td><?= eyc_admin_h($espacio['sede_nombre'] ?: ($espacio['clm_esp_idsede'] ?? 'Pendiente')) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
