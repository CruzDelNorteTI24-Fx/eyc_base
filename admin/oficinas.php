...
$tieneSedeEspacio = eyc_admin_has_column($conn, 'tb_espacio', 'clm_esp_idsede');
$espacios = eyc_admin_query_all($conn, $tieneSedeEspacio ? "
...
eyc_admin_render_head('Oficinas');
?>
<?php eyc_render_header(['title' => 'Oficinas', 'subtitle' => 'Administracion']); ?>
        ...
        <section class="admin-cat-office-grid" id="adminOfficeGrid">
            ...
            <?php foreach ($sedes as $sede): ?>
                ...
                foreach ($sedeEspacios as $espacio) {
                    ...
                    $searchParts[] = eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0);
                }
                ?>
                <article
                    class="admin-cat-office admin-cat-office--pro"
                    data-office-card
                    data-search="<?= eyc_admin_h(eyc_office_search_text($searchParts)) ?>"
                            ...
                            <div class="admin-cat-mini-list">
                                ...
                                <?php foreach ($sedeUsuarios as $usuario): ?>
                                    <span class="admin-cat-mini-item">
                                        <?= eyc_admin_h($usuario['nombre'] ?: $usuario['usuario']) ?>
                                        <small><?= eyc_admin_h($usuario['web_rol'] ?? 'Usuario') ?> · <?= eyc_admin_h($usuario['DNI'] ?: 'Sin DNI') ?></small>
                                    </span>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div class="admin-cat-office-column">
                            ...
                            <div class="admin-cat-mini-list">
                                ...
                                <?php foreach ($sedeAnaqueles as $anaquel): ?>
                                    <span class="admin-cat-mini-item">
                                        <?= eyc_admin_h($anaquel['clm_alm_anaquel_nombre'] ?? '') ?>
                                        <small><?= eyc_admin_h($anaquel['clm_alm_anaquel_codigo'] ?: 'Sin codigo') ?></small>
                                    </span>
                                <?php endforeach; ?>
                            </div>
                        </div>
                        <div class="admin-cat-office-column">
                            ...
                            <div class="admin-cat-mini-list">
                                ...
                                <?php foreach ($sedeEspacios as $espacio): ?>
                                    <span class="admin-cat-mini-item">
                                        <?= eyc_admin_h($espacio['clm_esp_nombre'] ?? '') ?> - <?= eyc_admin_h($espacio['clm_esp_desc'] ?? '') ?>
                                        <small><?= eyc_admin_h(eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0)) ?></small>
                                    </span>
                                <?php endforeach; ?>
                            </div>
                        </div>
            ...
            <section class="admin-cat-panel" id="adminOfficeGlobalSpaces">
                ...
                <div class="admin-cat-grid admin-cat-grid--inside">
                    <?php foreach ($espaciosSinSede as $espacio): ?>
                        <?php
                        $globalSearch = eyc_office_search_text([
                            $espacio['clm_esp_nombre'] ?? '',
                            $espacio['clm_esp_desc'] ?? '',
                            eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0),
                            $espacio['clm_esp_id'] ?? '',
                        ]);
                        ?>
                        <article class="admin-cat-card admin-cat-global-space" data-global-space data-search="<?= eyc_admin_h($globalSearch) ?>">
                            <h3><?= eyc_admin_h($espacio['clm_esp_nombre'] ?? '') ?></h3>
                            <p><?= eyc_admin_h($espacio['clm_esp_desc'] ?? '') ?></p>
                            <div class="admin-cat-card__meta">
                                <span class="admin-cat-chip"><?= eyc_admin_h(eyc_admin_space_module($espacio['clm_esp_obs'] ?? 0)) ?></span>
                                <span class="admin-cat-chip admin-cat-chip--muted">ID <?= eyc_admin_h($espacio['clm_esp_id'] ?? '') ?></span>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
