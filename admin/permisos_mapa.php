...
function eyc_ap_h($value): string {
    return htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8');
}

function eyc_ap_module_catalog(): array {
    $catalog = [
    ...
    foreach (eyc_menu_config() as $module) {
        ...
        $catalog[$id] = [
            ...
            'desc' => 'Modulo declarado en sidebar_eyc.php.',
        ];
    }
...
function eyc_ap_view_catalog(): array {
    $catalog = [
...
function eyc_ap_fetch_all(mysqli $conn, string $sql): array {
...
function eyc_ap_flash(string $type, string $message): void {
    $_SESSION['eyc_perm_flash'] = [
        'type' => $type,
        'message' => $message,
    ];
}

function eyc_ap_redirect(): void {
    header('Location: permisos_mapa.php');
    exit();
}

function eyc_ap_csrf_token(): string {
    if (empty($_SESSION['eyc_perm_token'])) {
        $_SESSION['eyc_perm_token'] = bin2hex(random_bytes(16));
    }

    return (string)$_SESSION['eyc_perm_token'];
}

function eyc_ap_validate_csrf(): bool {
    $token = (string)($_POST['eyc_perm_token'] ?? '');
    $sessionToken = (string)($_SESSION['eyc_perm_token'] ?? '');

    return $token !== '' && $sessionToken !== '' && hash_equals($sessionToken, $token);
}

function eyc_ap_normalized_view($value): ?string {
...
function eyc_ap_allowed_types(): array {
    return ['lectura', 'lectura/escritura'];
}

function eyc_ap_find_permission(mysqli $conn, int $userId, int $moduleId, ?string $view): int {
    if ($view === null) {
        $stmt = $conn->prepare("
            SELECT id_permiso
            FROM tb_permisos
...
function eyc_ap_upsert_permission(mysqli $conn, int $userId, int $moduleId, ?string $view, string $type): string {
    $existingId = eyc_ap_find_permission($conn, $userId, $moduleId, $view);
...
function eyc_ap_user_label(array $user): string {
...
function eyc_ap_names(array $ids, array $users): array {
    $names = [];
    foreach ($ids as $id) {
        if (isset($users[$id])) $names[] = eyc_ap_user_label($users[$id]);
    }
    natcasesort($names);
    return array_values($names);
}

function eyc_ap_required_rule(array $item, int $moduleId): array {
...
$moduleCatalog = eyc_ap_module_catalog();
$viewCatalog = eyc_ap_view_catalog();
$users = [];
$adminIds = [];
$moduleUsers = [];
$viewUsers = [];

$userRows = eyc_ap_fetch_all($conn, "SELECT id_usuario, usuario, nombre, web_rol, clm_usuarios_sede FROM tb_usuarios ORDER BY web_rol='Admin' DESC, nombre, usuario");
...
$csrfToken = eyc_ap_csrf_token();
$flash = $_SESSION['eyc_perm_flash'] ?? null;
unset($_SESSION['eyc_perm_flash']);

if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    if (!eyc_ap_validate_csrf()) {
        eyc_ap_flash('error', 'No se pudo validar la accion. Actualiza la pagina e intenta nuevamente.');
        eyc_ap_redirect();
    }

    $action = (string)($_POST['action'] ?? '');
    $allowedTypes = eyc_ap_allowed_types();
    ...
    if ($action === 'create') {
        ...
        $view = eyc_ap_normalized_view($_POST['vista_redirect'] ?? '');
        ...
        if (empty($userIds)) {
            eyc_ap_flash('error', 'Selecciona al menos un usuario para asignar el permiso.');
            eyc_ap_redirect();
        }

        if (!isset($moduleCatalog[$moduleId])) {
            eyc_ap_flash('error', 'Selecciona un modulo valido para el permiso.');
            eyc_ap_redirect();
        }
        ...
        foreach (array_keys($userIds) as $userId) {
            $result = eyc_ap_upsert_permission($conn, $userId, $moduleId, $view, $type);
        ...
        eyc_ap_flash($errors > 0 ? 'warning' : 'success', 'Gestion de permisos aplicada: ' . implode(', ', $parts) . '.');
        eyc_ap_redirect();
    }
    ...
    if ($action === 'update') {
        ...
        if ($permissionId <= 0 || $userId <= 0 || !isset($users[$userId]) || !isset($moduleCatalog[$moduleId])) {
            eyc_ap_flash('error', 'No se pudo actualizar el permiso porque los datos no son validos.');
            eyc_ap_redirect();
        }
        ...
        if (!$stmt) {
            eyc_ap_flash('error', 'No se pudo preparar la actualizacion del permiso.');
            eyc_ap_redirect();
        }
        ...
        eyc_ap_flash($ok ? 'success' : 'error', $ok ? 'Permiso actualizado correctamente.' : 'No se pudo actualizar el permiso.');
        eyc_ap_redirect();
    }
    ...
    if ($action === 'delete') {
        $permissionId = (int)($_POST['id_permiso'] ?? 0);
        if ($permissionId <= 0) {
            eyc_ap_flash('error', 'No se pudo identificar el permiso a eliminar.');
            eyc_ap_redirect();
        }

        $stmt = $conn->prepare("DELETE FROM tb_permisos WHERE id_permiso = ? LIMIT 1");
        if (!$stmt) {
            eyc_ap_flash('error', 'No se pudo preparar la eliminacion del permiso.');
            eyc_ap_redirect();
        }
        ...
        eyc_ap_flash(($ok && $affected > 0) ? 'success' : 'warning', ($ok && $affected > 0) ? 'Permiso eliminado correctamente.' : 'El permiso ya no existe o no pudo eliminarse.');
        eyc_ap_redirect();
    }

    eyc_ap_flash('error', 'Accion de permisos no reconocida.');
    eyc_ap_redirect();
}

$permissionRows = eyc_ap_fetch_all($conn, "
...
foreach (eyc_menu_config() as $module) {
    ...
    foreach (($module['grupos'] ?? []) as $group) {
        foreach (($group['items'] ?? []) as $item) {
            ...
            $rule = eyc_ap_required_rule($item, $itemModule);
            ...
            $interfaces[] = [
                ...
                'url' => (string)($item['url'] ?? ''),
                'rule' => $rule,
                'users' => eyc_ap_names(array_keys($accessIds), $users),
            ];
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Mapa de permisos | Eyc</title>
    ...
    <link rel="stylesheet" href="<?= eyc_asset('assets/css/admin_permisos_eyc.css') ?>">
</head>
<body>
...
<main class="main-content eyc-main eyc-main--module eyc-main--compact-access" role="main">
    <div class="eyc-main__inner admin-perms-shell">
        ...
        <section class="admin-perms-hero">
            ...
            <a href="<?= eyc_ap_h(eyc_base_url('index.php')) ?>" class="admin-perms-btn"><i class="bi bi-arrow-left" aria-hidden="true"></i> Panel principal</a>
        </section>

        <section class="admin-perms-summary" aria-label="Resumen de permisos">
            <div><span>Usuarios</span><strong><?= eyc_ap_h($totalUsers) ?></strong></div>
            <div><span>Administradores</span><strong><?= eyc_ap_h($totalAdmins) ?></strong></div>
            <div><span>Asignaciones</span><strong><?= eyc_ap_h($totalPermissionRows) ?></strong></div>
            <div><span>Interfaces mapeadas</span><strong><?= eyc_ap_h($totalInterfaces) ?></strong></div>
        </section>

        <?php if (is_array($flash)): ?>
            <div class="admin-perms-alert admin-perms-alert--<?= eyc_ap_h($flash['type'] ?? 'success') ?>">
                <i class="bi bi-info-circle-fill" aria-hidden="true"></i>
                <span><?= eyc_ap_h($flash['message'] ?? '') ?></span>
            </div>
        <?php endif; ?>
        ...
        <section class="admin-perms-panel admin-perms-manage" aria-labelledby="managePermsTitle">
            ...
            <form method="post" class="admin-perms-form" id="adminPermsCreateForm">
                <input type="hidden" name="eyc_perm_token" value="<?= eyc_ap_h($csrfToken) ?>">
                ...
                <label>
                    ...
                    <select name="id_usuario[]" multiple required size="7">
                        <?php foreach ($users as $userId => $user): ?>
                            <option value="<?= eyc_ap_h($userId) ?>"><?= eyc_ap_h(eyc_ap_user_label($user)) ?> - <?= eyc_ap_h($user['web_rol'] ?? 'Usuario') ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <label>
                    ...
                    <select name="id_modulo" id="adminPermsModuleSelect" required>
                        <?php foreach ($moduleCatalog as $moduleId => $module): ?>
                            <option value="<?= eyc_ap_h($moduleId) ?>"><?= eyc_ap_h($moduleId) ?> - <?= eyc_ap_h($module['title']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
                ...
                <label>
                    ...
                    <select name="tipo_permiso">
                        <?php foreach (eyc_ap_allowed_types() as $type): ?>
                            <option value="<?= eyc_ap_h($type) ?>"><?= eyc_ap_h($type) ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
            ...
            <datalist id="adminPermsViewCodes">
                <?php foreach ($viewCatalog as $code => $view): ?>
                    <option value="<?= eyc_ap_h($code) ?>"><?= eyc_ap_h($view['title']) ?></option>
                <?php endforeach; ?>
            </datalist>
        </section>
        ...
        <section class="admin-perms-panel" aria-labelledby="interfacesTitle">
            <div class="admin-perms-panel-head">
                ...
                <span><?= eyc_ap_h($totalInterfaces) ?> interfaces</span>
            </div>
            <div class="admin-perms-table-wrap">
                <table class="admin-perms-table" id="adminPermsTable">
                    ...
                    <?php foreach ($interfaces as $interface): ?>
                        <tr>
                            <td><strong><?= eyc_ap_h($interface['module']) ?></strong><small>ID <?= eyc_ap_h($interface['module_id']) ?> / <?= eyc_ap_h($interface['group']) ?></small></td>
                            <td><strong><?= eyc_ap_h($interface['title']) ?></strong><small><?= eyc_ap_h($interface['url']) ?></small></td>
                            <td>
                                <span class="admin-perms-rule"><?= eyc_ap_h($interface['rule']['label']) ?></span>
                                <?php if (!empty($interface['rule']['codes'])): ?>
                                    <div class="admin-perms-code-list">
                                        <?php foreach ($interface['rule']['codes'] as $code): ?><code><?= eyc_ap_h($code) ?></code><?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
                                <button type="button"
                                        class="admin-perms-mini-btn"
                                        data-perm-fill
                                        data-module="<?= eyc_ap_h($interface['module_id']) ?>"
                                        data-view="<?= eyc_ap_h($interface['rule']['codes'][0] ?? '') ?>">
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <div class="admin-perms-user-list">
                                        <?php foreach ($interface['users'] as $name): ?><span><?= eyc_ap_h($name) ?></span><?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
                            </td>
                        </tr>
        ...
        <section class="admin-perms-grid">
            <article class="admin-perms-panel">
                <div class="admin-perms-panel-head"><h2>Modulos</h2><span><?= eyc_ap_h(count($moduleCatalog)) ?> registros</span></div>
                <div class="admin-perms-list">
                    <?php foreach ($moduleCatalog as $id => $module): ?>
                        <div class="admin-perms-list-item"><code><?= eyc_ap_h($id) ?></code><span><strong><?= eyc_ap_h($module['title']) ?></strong><small><?= eyc_ap_h($module['desc']) ?></small></span></div>
                    <?php endforeach; ?>
                </div>
            </article>

            <article class="admin-perms-panel">
                <div class="admin-perms-panel-head"><h2>Codigos de vista</h2><span><?= eyc_ap_h(count($viewCatalog)) ?> codigos</span></div>
                <div class="admin-perms-list">
                    <?php foreach ($viewCatalog as $code => $view): ?>
                        <div class="admin-perms-list-item"><code><?= eyc_ap_h($code) ?></code><span><strong><?= eyc_ap_h($view['title']) ?></strong><small>Modulo <?= eyc_ap_h($view['module']) ?> / <?= eyc_ap_h($view['desc']) ?></small></span></div>
                    <?php endforeach; ?>
                </div>
            </article>
        </section>
        ...
        <section class="admin-perms-panel" aria-labelledby="rawPermsTitle">
            <div class="admin-perms-panel-head"><h2 id="rawPermsTitle">Asignaciones actuales en tb_permisos</h2><span><?= eyc_ap_h($totalPermissionRows) ?> filas</span></div>
                        ...
                        <?php foreach ($permissionRows as $row): ?>
                            ...
                            <tr>
                                <td class="admin-perms-actions">
                                    <form method="post" id="<?= eyc_ap_h($editFormId) ?>">
                                        <input type="hidden" name="eyc_perm_token" value="<?= eyc_ap_h($csrfToken) ?>">
                                        <input type="hidden" name="action" value="update">
                                        <input type="hidden" name="id_permiso" value="<?= eyc_ap_h($permissionId) ?>">
                                    </form>
                                    <form method="post" id="<?= eyc_ap_h($deleteFormId) ?>" onsubmit="return confirm('Seguro que deseas eliminar este permiso?');">
                                        <input type="hidden" name="eyc_perm_token" value="<?= eyc_ap_h($csrfToken) ?>">
                                        <input type="hidden" name="action" value="delete">
                                        <input type="hidden" name="id_permiso" value="<?= eyc_ap_h($permissionId) ?>">
                                    </form>
                                    <button type="submit" form="<?= eyc_ap_h($editFormId) ?>" class="admin-perms-icon-btn" title="Guardar cambios">
                                        <i class="bi bi-save-fill" aria-hidden="true"></i>
                                    </button>
                                    <button type="submit" form="<?= eyc_ap_h($deleteFormId) ?>" class="admin-perms-icon-btn admin-perms-icon-btn--danger" title="Eliminar permiso">
                                        <i class="bi bi-trash3-fill" aria-hidden="true"></i>
                                    </button>
                                </td>
                                <td>
                                    <select name="id_usuario" form="<?= eyc_ap_h($editFormId) ?>" class="admin-perms-cell-control">
                                        <?php foreach ($users as $userId => $user): ?>
                                            <option value="<?= eyc_ap_h($userId) ?>" <?= (int)$row['id_usuario'] === (int)$userId ? 'selected' : '' ?>><?= eyc_ap_h(eyc_ap_user_label($user)) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </td>
                                <td><?= eyc_ap_h($row['web_rol'] ?? 'Usuario') ?></td>
                                <td><?= eyc_ap_h($row['clm_usuarios_sede'] ?? 'No asignada') ?></td>
                                <td>
                                    <select name="id_modulo" form="<?= eyc_ap_h($editFormId) ?>" class="admin-perms-cell-control">
                                        <?php foreach ($moduleCatalog as $catalogModuleId => $module): ?>
                                            <option value="<?= eyc_ap_h($catalogModuleId) ?>" <?= $moduleId === (int)$catalogModuleId ? 'selected' : '' ?>><?= eyc_ap_h($catalogModuleId) ?> - <?= eyc_ap_h($module['title']) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </td>
                                <td>
                                    <input type="text"
                                           name="vista_redirect"
                                           form="<?= eyc_ap_h($editFormId) ?>"
                                           list="adminPermsViewCodes"
                                           class="admin-perms-cell-control"
                                           value="<?= eyc_ap_h($viewCode) ?>"
                                           placeholder="Sin vista">
                                </td>
                                <td>
                                    <select name="tipo_permiso" form="<?= eyc_ap_h($editFormId) ?>" class="admin-perms-cell-control">
                                        <?php foreach (eyc_ap_allowed_types() as $type): ?>
                                            <option value="<?= eyc_ap_h($type) ?>" <?= (string)($row['tipo_permiso'] ?? '') === $type ? 'selected' : '' ?>><?= eyc_ap_h($type) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </td>
                                <td><?= eyc_ap_h($row['fecha_asignacion'] ?? '') ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    </tbody>
