...
$permisosPorUsuario = eyc_admin_permissions_by_user($conn);
...
eyc_admin_render_head('Usuarios');
?>
<?php eyc_render_header(['title' => 'Usuarios', 'subtitle' => 'Administracion']); ?>
            ...
            <div class="admin-cat-table-wrap">
                <table class="admin-cat-table">
                    ...
                    <?php foreach ($usuarios as $usuario): ?>
                        ...
                        $photo = eyc_admin_photo_data_uri($usuario['foto_usuario'] ?: ($usuario['foto_trabajador'] ?? ''));
                            ...
                            <td>
                                <div class="admin-cat-user">
                                    <span class="admin-cat-avatar">
                                        <?php if ($photo !== ''): ?>
                                            <img src="<?= $photo ?>" alt="Foto de <?= eyc_admin_h($displayName) ?>">
                                        <?php else: ?>
                                            <?= eyc_admin_h(eyc_admin_initials($displayName)) ?>
                                        <?php endif; ?>
                                    </span>
                                    <span>
                                        <strong><?= eyc_admin_h($displayName) ?></strong>
                                        <span>@<?= eyc_admin_h($username) ?> · ID <?= $userId ?></span>
                                    </span>
                                </div>
                            </td>
                            <td><span class="admin-cat-chip <?= $isAdmin ? 'admin-cat-chip--admin' : '' ?>"><?= eyc_admin_h($usuario['web_rol'] ?? 'Usuario') ?></span></td>
                            <td><?= eyc_admin_h($usuario['DNI'] ?? 'No registrado') ?></td>
                            <td><?= eyc_admin_h($usuario['sede_nombre'] ?: ($usuario['clm_usuarios_sede'] ?? 'No asignada')) ?></td>
                            <td>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($permisos as $permiso): ?>
                                            <span class="admin-cat-chip">
                                                <?= eyc_admin_h($permiso['module']) ?> · <?= eyc_admin_h($permiso['view']) ?>
                                            </span>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
