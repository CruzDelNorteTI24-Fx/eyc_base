(function () {
  ...
  const alertBox = (message, variant = 'info', title = '') => {
    if (window.eycDialog) {
      return window.eycDialog.alert(message, { variant, title });
    }
  ...
  const confirmBox = (message, options = {}) => {
    if (window.eycDialog) {
      return window.eycDialog.confirm(message, options);
    }
  ...
  const withLoader = async (task, options = {}) => {
    if (window.eycLoader) {
      return window.eycLoader.during(task, options);
    }
    return task();
  };
  ...
  $('#almEntradaForm')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (data.auto_pdf && data.nota_id && window.eycNotaPDF) {
        await window.eycNotaPDF.downloadByNotaId(data.nota_id);
      }
