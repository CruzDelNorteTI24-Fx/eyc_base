(function () {
    if (window.eycBarcodeReady) return;
    window.eycBarcodeReady = true;
    ...
    const makeSvg = (code) => {
        ...
        return `
            <svg class="eyc-barcode-svg" viewBox="0 0 ${modules} 100" preserveAspectRatio="none" role="img" aria-label="Codigo de barras ${esc(code)}" xmlns="http://www.w3.org/2000/svg">
                <rect x="0" y="0" width="${modules}" height="100" fill="#fff"></rect>
                ${bars.join('')}
                <text x="${modules / 2}" y="94" text-anchor="middle" font-family="eycConsola, Consolas, Courier New, monospace" font-size="16" font-weight="900" fill="#000">${esc(code)}</text>
            </svg>
        `;
    };
    ...
    const drawBarcodeCanvas = (ctx, code, x0, y0, width, height) => {
        ...
        ctx.font = '900 25px eycConsola, Consolas, Courier New, monospace';
    ...
    const fitText = (ctx, text, maxWidth, maxSize, minSize) => {
        ...
        for (; size >= minSize; size -= 1) {
            ctx.font = `700 ${size}px eycConsola, Consolas, Courier New, monospace`;
            if (ctx.measureText(output).width <= maxWidth) {
                return { text: output, size };
            }
        }

        ctx.font = `700 ${minSize}px eycConsola, Consolas, Courier New, monospace`;
    ...
    const downloadPng = async (el) => {
        ...
        ctx.font = `700 ${cat.size}px eycConsola, Consolas, Courier New, monospace`;
        ctx.fillText(cat.text, xBarcode, y);
        y += 16 + textGap2;

        const name = fitText(ctx, data.name, maxTextW, 14, 8);
        ctx.font = `700 ${name.size}px eycConsola, Consolas, Courier New, monospace`;
    ...
    const renderElement = (el) => {
        ...
        try {
            stage.innerHTML = `
                <div class="eyc-barcode-paper">
                    ${makeSvg(data.code)}
                    <div class="eyc-barcode-text">
            ...
            stage.innerHTML = '<div class="eyc-barcode-error">No se pudo generar la previsualizacion del codigo.</div>';
        }
    };

    const renderAll = (root) => {
        const scope = root || document;
        scope.querySelectorAll('[data-eyc-barcode]').forEach(renderElement);
    };
    ...
    document.addEventListener('click', (event) => {
        ...
        const card = button.closest('[data-eyc-barcode]');
        if (!card) return;
