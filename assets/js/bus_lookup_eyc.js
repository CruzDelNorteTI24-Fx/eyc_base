(function () {
  const modal = document.getElementById('eycBusLookupModal');
  if (!modal || modal.dataset.ready === '1') return;

  modal.dataset.ready = '1';

  const endpoint = modal.dataset.endpoint || '';
  const form = document.getElementById('eycBusLookupForm');
  const input = document.getElementById('eycBusLookupInput');
  const statusEl = document.getElementById('eycBusLookupStatus');
  const resultEl = document.getElementById('eycBusLookupResult');
  const openers = document.querySelectorAll('[data-eyc-bus-open]');
  const closers = modal.querySelectorAll('[data-eyc-bus-close]');
  ...
  const open = (seed = '') => {
    ...
    document.body.classList.add('eyc-modal-open');
  ...
  const close = () => {
    ...
    document.body.classList.remove('eyc-modal-open');
  };

  const section = (title, count, html) => `
    <section class="eyc-bus-section">
      <div class="eyc-bus-section__head">
        <h3>${esc(title)}</h3>
        <span>${esc(count)}</span>
      </div>
      ${html}
  ...
  const renderEmpty = (message) => `<div class="eyc-bus-empty">${esc(message)}</div>`;
  ...
  const renderSuggestions = (data) => {
    ...
    resultEl.innerHTML = `
      <div class="eyc-bus-suggestions">
        ${items.map((bus) => `
          <button class="eyc-bus-suggestion" type="button" data-bus-id="${esc(bus.id || bus.id_bus)}">
  ...
  const renderDetail = (data) => {
    ...
    const conductoresHtml = conductores.length ? `
      <div class="eyc-bus-list">
        ${conductores.map((item) => `
          <div class="eyc-bus-row">
            <span class="eyc-bus-row__icon"><i class="bi bi-person-vcard-fill"></i></span>
            <span class="eyc-bus-row__main">
              <strong>${esc(text(item.conductor, 'Conductor sin nombre'))}</strong>
              <small>DNI ${esc(text(item.dni))} | Lic. ${esc(text(item.licencia))}</small>
            </span>
            <span class="eyc-bus-row__tag">${esc(text(item.fecha_asignacion || item.fecha_programacion, 'Asignacion activa'))}</span>
          </div>
        `).join('')}
      </div>
    ...
    const horariosHtml = horarios.length ? `
      <div class="eyc-bus-list">
        ${horarios.map((item) => `
          <div class="eyc-bus-row">
            <span class="eyc-bus-row__icon"><i class="bi bi-signpost-split-fill"></i></span>
            <span class="eyc-bus-row__main">
            ...
            <span class="eyc-bus-row__tag">${esc(text(item.fecha_operativa, 'Operativo actual'))}</span>
          </div>
        `).join('')}
      </div>
    ...
    const checklistsHtml = checklists.length ? `
      <div class="eyc-bus-list">
        ${checklists.map((item) => `
          <div class="eyc-bus-row">
            <span class="eyc-bus-row__icon"><i class="bi bi-clipboard2-check-fill"></i></span>
            <span class="eyc-bus-row__main">
            ...
            <span class="eyc-bus-row__tag">${esc(text(item.fecha))} ${esc(text(item.hora, ''))}</span>
          </div>
        `).join('')}
      </div>
    ...
    resultEl.innerHTML = `
      <article class="eyc-bus-card">
        <div class="eyc-bus-card__hero">
          <div>
            <h3 class="eyc-bus-card__title" style="color: #ffffff;">${esc(text(bus.nombre || bus.bus, 'Unidad'))}</h3>
            <p class="eyc-bus-card__subtitle">
              ${esc(text(bus.servicio, 'Servicio sin registrar'))} | ${esc(text(bus.tipo, 'Tipo sin registrar'))} | ${esc(text(bus.dueno, 'Dueno sin registrar'))}
            </p>
          </div>
          <div class="eyc-bus-card__plate">
            <small>Placa</small>
            <strong>${esc(text(bus.placa))}</strong>
          </div>
        </div>

        <div class="eyc-bus-grid">
          <div class="eyc-bus-kpi"><span>Conductores</span><strong>${esc(resumen.conductores ?? conductores.length)}</strong></div>
          <div class="eyc-bus-kpi"><span>Horarios activos</span><strong>${esc(resumen.horarios ?? horarios.length)}</strong></div>
          <div class="eyc-bus-kpi"><span>Checklist recientes</span><strong>${esc(resumen.checklists ?? checklists.length)}</strong></div>
          ${fumigacionHtml}
        </div>
