(function (window) {
    ...
    function common() {
        if (!window.eycNotasCommon) {
            throw new Error('eycNotasCommon no esta cargado.');
        }
        return window.eycNotasCommon;
    }
    ...
    function normalize(cfg, data) {
        ...
        return C.baseNote(cfg, data, {
            ...
            reason: '',
            withAmounts: true,
            fileName: 'eyc_formato_AB_abastecimiento_combustible.pdf'
        });
    }
    ...
    async function generateDemo(options) {
        if (!window.eycNotasDemoData) {
            throw new Error('eycNotasDemoData no esta cargado.');
        }

        const cfg = Object.assign({}, options || {}, {
            noteData: window.eycNotasDemoData.get('AB', options || {})
        });
        return generate(cfg);
    }

    window.eycNotaAbastecimiento = {
        generate,
        generateDemo
    };
})(window);
