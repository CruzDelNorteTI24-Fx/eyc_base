(function (window, document) {
  ...
  const CFG = window.eyc_CHECK_REPORT || {};
  ...
  async function during(fn, options) {
    if (window.eycLoader && typeof window.eycLoader.during === 'function') {
      return window.eycLoader.during(fn, options || {});
    }
    return fn();
  }
  ...
  async function generateChecklistPdf(detail) {
    if (!window.eycPDF) throw new Error('eycPDF no esta cargado.');
    const chk = detail.checklist || {};
    const doc = await window.eycPDF.createDocument(basePdfConfig({
