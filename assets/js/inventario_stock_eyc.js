(function (window, document) {
    ...
    const config = window.eyc_STOCK_REPORT || {};
    ...
    function ensureToastStyles() {
        if (document.getElementById('eyc-stock-toast-style')) return;

        const style = document.createElement('style');
        style.id = 'eyc-stock-toast-style';
        style.textContent = `
            .eyc-note-pdf-toast {
                position: fixed;
                right: 22px;
            ...
            .eyc-note-pdf-toast[data-type="error"] { background: #b91c1c; }
            .eyc-note-pdf-toast[data-type="success"] { background: #047857; }
            .eyc-note-pdf-toast[hidden] { display: none; }
        `;
    ...
    function verMovimientos(idProducto) {
        ...
        fetch(url, { credentials: 'same-origin' })
            ...
            .then(html => {
                ...
                if (window.eycBarcode && typeof window.eycBarcode.renderAll === 'function') {
                    window.eycBarcode.renderAll(content);
                }
            })
            .catch(error => {
                console.error(error);
    ...
    async function exportPdf(button) {
        ...
        if (!window.eycPDF || !window.jspdf || !window.jspdf.jsPDF) {
        ...
        try {
            ...
            const doc = await window.eycPDF.createDocument({
                ...(config.pdf || {}),
                useCover: false,
