(function (window, document) {
    ...
    const config = window.eyc_KARDEX_REPORT || {};
    ...
    function ensureToastStyles() {
        if (document.getElementById('eyc-kardex-toast-style')) return;

        const style = document.createElement('style');
        style.id = 'eyc-kardex-toast-style';
        style.textContent = `
            .eyc-kardex-toast {
                position: fixed;
                right: 22px;
            ...
            .eyc-kardex-toast[data-type="error"] { background: #b91c1c; }
            .eyc-kardex-toast[data-type="success"] { background: #047857; }
            .eyc-kardex-toast[hidden] { display: none; }
        `;
    ...
    function toast(message, type) {
        ensureToastStyles();
        let box = $('.eyc-kardex-toast');

        if (!box) {
            box = document.createElement('div');
            box.className = 'eyc-kardex-toast';
