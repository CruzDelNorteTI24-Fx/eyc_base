(function (window, document) {
    ...
    function ensureLoader() {
        ...
        root.className = 'eyc-loader';
        root.id = 'eycLoader';
        root.setAttribute('role', 'status');
        root.setAttribute('aria-live', 'polite');
        root.setAttribute('aria-hidden', 'true');
        root.innerHTML = [
            '<div class="eyc-loader__card">',
            '  <div class="eyc-loader__mark"><div class="eyc-loader__spinner" aria-hidden="true"></div></div>',
            '  <p class="eyc-loader__title">Procesando...</p>',
            '  <p class="eyc-loader__detail">Por favor espere</p>',
            '  <div class="eyc-loader__bar" aria-hidden="true"></div>',
            '</div>'
        ].join('');

        document.body.appendChild(root);
        titleEl = root.querySelector('.eyc-loader__title');
        detailEl = root.querySelector('.eyc-loader__detail');
        return root;
    }
    ...
    function setButtonBusy(button, busy) {
        ...
        if (busy) {
            ...
            target.classList.add('eyc-is-loading');
            return;
        }

        const prev = busyButtons.get(target);
        target.classList.remove('eyc-is-loading');
        if (prev) {
    ...
    function show(options) {
        ...
        document.body.classList.add('eyc-loader-open');

        return Symbol('eyc-loader-token');
    }
    ...
    function hide() {
        ...
        root.setAttribute('aria-hidden', 'true');
        document.body.classList.remove('eyc-loader-open');
    }
