(function (window, document) {
    ...
    const config = window.eyc_NOTA_PDF_CONFIG || {};
    ...
    function toast(message, type) {
        if (!document.querySelector('#eyc-note-pdf-style')) {
            const style = document.createElement('style');
            style.id = 'eyc-note-pdf-style';
            style.textContent = `
                .eyc-note-pdf-toast{
                    position:fixed;
                    right:22px;
                ...
                .eyc-note-pdf-toast[data-type="error"]{background:#991b1b;}
                .eyc-note-pdf-toast[hidden]{display:none!important;}
            `;
            document.head.appendChild(style);
        }

        let box = document.querySelector('.eyc-note-pdf-toast');

        if (!box) {
            box = document.createElement('div');
            box.className = 'eyc-note-pdf-toast';
    ...
    function setBusy(button, busy) {
        ...
        if (busy) {
            ...
            return () => {
                ...
                delete button.dataset.eycPdfBusy;
            };
        }
    ...
    document.addEventListener('click', (event) => {
        const button = event.target.closest('[data-eyc-note-download]');

        if (!button || button.dataset.eycPdfBusy === '1') {
            return;
        }
